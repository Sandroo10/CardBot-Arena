...
export const profileShell = cva("min-h-screen py-12")

export const profileContent = cva("space-y-10")

export const profileHeader = cva(
  "grid gap-6 rounded-[1.75rem] border border-white/10 bg-background/78 p-6 shadow-[0_18px_42px_rgba(0,0,0,0.22)] md:grid-cols-[auto_1fr_auto] md:items-center"
)

export const profileAvatar = cva(
  "grid size-24 place-items-center rounded-2xl border border-bura/35 bg-bura/15 text-3xl font-black text-bura shadow-[0_16px_34px_rgba(0,0,0,0.24)]"
)
...
export const profileName = cva("text-4xl font-black leading-tight text-foreground sm:text-5xl")

export const profileDescription = cva("max-w-2xl text-base leading-7 text-muted-foreground")

export const profileRank = cva(
  "w-fit rounded-2xl border border-primary/30 bg-primary/10 px-5 py-4 text-center"
)

export const profileRankLabel = cva("text-xs font-black uppercase tracking-[0.16em] text-muted-foreground")

export const profileRankValue = cva("mt-1 text-3xl font-black text-primary")

export const statsGrid = cva("grid gap-4 sm:grid-cols-2 lg:grid-cols-4")
  ...
  "grid grid-cols-[3rem_1fr] gap-4 rounded-2xl border p-4",
  {
    variants: {
      unlocked: {
        ...
        false: "border-white/8 bg-white/5 opacity-65 grayscale"
      }
    },
    defaultVariants: {
      unlocked: false
  ...
  "grid size-12 place-items-center rounded-xl border",
  {
    variants: {
      unlocked: {
        ...
        false: "border-white/10 bg-white/8 text-muted-foreground"
      }
    },
    defaultVariants: {
      unlocked: false
...
export const achievementTitle = cva("font-black text-foreground")
